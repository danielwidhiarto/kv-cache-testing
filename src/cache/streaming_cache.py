"""KV cache with StreamingLLM eviction (attention sink + window)."""

import torch
from typing import Optional, Tuple
from .base import KVCache
from ..policies.streaming import StreamingPolicy


class StreamingCache(KVCache):
    """KV cache that keeps attention sinks + sliding window."""

    def __init__(self, max_size: int = 2048, sink_size: int = 4, window_size: int = 512):
        self._max_size = max_size
        self._keys: Optional[torch.Tensor] = None
        self._values: Optional[torch.Tensor] = None
        self._policy = StreamingPolicy(sink_size=sink_size, window_size=window_size)
