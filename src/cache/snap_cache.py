"""KV cache with SnapKV eviction (per-head attention-based)."""

import torch
from typing import Optional, Tuple
from .base import KVCache
from ..policies.snap import SnapPolicy


class SnapCache(KVCache):
    """KV cache that evicts based on per-head attention patterns."""

    def __init__(self, max_size: int = 2048, observation_size: int = 32, retain_ratio: float = 0.5):
        self._max_size = max_size
        self._keys: Optional[torch.Tensor] = None
        self._values: Optional[torch.Tensor] = None
        self._policy = SnapPolicy(observation_size=observation_size, retain_ratio=retain_ratio)
